...
struct SkipContext {
  SkipContext(std::string &read, reference_index *pfi_in, int32_t k_in,
    ...
    : kit1(read), kit_tmp(read), pfi(pfi_in),
      ...
      miss_it(0), global_contig_pos(-1), alt_skip(alt_skip_in),
      hit_found(false) {}
  ...
  inline void set_iter(pufferfish::CanonicalKmerIterator &kit_in) {
    kit1 = kit_in;
  }
  ...
  inline bool query_kmer_complex(piscem::streaming_query &qc,
    ...
    if (fast_hit.valid()) {
      ...
      if (keq != KmerMatchType::NO_MATCH) {
        ...
        // confirmatory hit.
        // Note: consider if the below should actually be "&=" instead of "=" or
        // not.
  ...
  inline int32_t compute_safe_skip() {
    // read_target_pos = //hit_is_before_target_pos() ? read_target_pos :
    // read_target_pos-1;
  ...
  inline void fast_forward() {
    ...
    // but, this can be violated if we skipped farther than we asked in the
    // iterator because there were 'N's in the read. in that case case, ignore
    // the fact that fast forward could be a rewind.  We always want to skip
    // back to where we were before.  However, in this case we ASSUME that no
    // hit *after* kit_tmp has been added to the set of collected hits.
    kit1 = kit_tmp;
  }
  ...
  inline void advance_from_hit(bool switching_unitig = false) {
    ...
    if (phits.contigOrientation_) {
      ctg_skip =
        static_cast<int64_t>(cEndPos) - (static_cast<int64_t>(cCurrPos + k));
    } else { // rc ori
    ...
    last_skip_type = (read_skip <= ctg_skip) ? LastSkipType::SKIP_READ
                                             : LastSkipType::SKIP_UNI;
    ...
    read_target_pos =
      (kit1->second > read_len - k) ? read_len - k : kit1->second;
    clear_miss_counter();
    ...
    if (expected_skip and (expected_cid != invalid_cid) and (kit1 != kit_end)) {
      ...
      ref_contig_it.size() << "\n"; std::cout << "expected_cid = " <<
      expected_cid << ", skip = " << skip << "\n";
      }
      */
  ...
  inline void advance_from_miss() {
    ...
    case LastSkipType::SKIP_UNI: {
      ...
      if (dist <= 1) {
        ...
        // debug print
        // std::cerr << "dist = " << dist << ", read_target_pos = " <<
        // read_target_pos
...
inline void walk_safely_until(
  ...
  while (!skip_ctx.is_exhausted() && skip_ctx.read_pos() <= end_read_pos) {
    ...
    if (skip_ctx.is_good_query() and skip_ctx.query_kmer(qc)) {
      ...
      // until we reach the end of the read, the end of the contig, or a
      // mismatch.
      ...
      if (last_valid_hit.first > raw_hits.back().first) {
        ...
        if (!(skip_ctx.read_pos() > last_valid_hit.first)) {
                std::cerr << "skip_ctx.read_pos() = " << skip_ctx.read_pos() <<
        "; last_valid_hit pos = " << last_valid_hit.first << "; raw_hits.back()
        = " << raw_hits.back().first << "\n";
        }
        */
        raw_hits.push_back(last_valid_hit);
        // assert(("read_pos > last_valid_hit.first", skip_ctx.read_pos() >
        // last_valid_hit.first));
      }
...
inline bool
check_direct_match(SkipContext &skip_ctx, int32_t k, int direction,
                   int32_t dist, int64_t &curr_pos,
...
struct EveryKmer {
  ...
  EveryKmer(int32_t k)
    : cStartPos(std::numeric_limits<int64_t>::max()),
      ...
      dist_to_contig_end(std::numeric_limits<int64_t>::max()), k(k),
      new_state(true) {}

  EveryKmer(projected_hits ph, size_t k)
    : cStartPos(static_cast<int64_t>(ph.globalPos_ - ph.contigPos_)),
      cEndPos(
        static_cast<int64_t>(ph.globalPos_ - ph.contigPos_ + ph.contigLen_)),
  ...
  inline void query_kmer(pufferfish::CanonicalKmerIterator &kit,
    ...
    if (!ph.empty()) {
      // std::cerr << "found hit between " << kit->second << " and " <<
      // ph.globalPos_ << "\n";
  ...
  inline bool check_match(sshash::bit_vector_iterator &ref_contig_it,
    ...
    and " << cpos << "\n"; auto k2 = kit->first; k2.fromNum(ref_kmer); std::cerr
    << "\t\t ref_kmer = " << k2.to_str() << ", kit = " << kit->first.to_str() <<
    "\n";
    */
  ...
  if (strict_mode) {
    ...
    while (!skip_ctx.is_exhausted()) {
      ...
      if (skip_ctx.is_good_query() and skip_ctx.query_kmer(qc)) {
        ...
        // (i.e. it's read position is greater than any hit we have seen
        // before), then add it.
        if (raw_hits.empty() or (read_pos > raw_hits.back().first)) {
          auto proj_hits = skip_ctx.proj_hits();
          // this hit was not looking for a match on a known contig
        ...
        // If we can successfully jump to the end of this unitig then just do
        // it.
        int64_t dist_to_read_end = (read.size() - k) - read_pos;
        int32_t skip_dist =
          static_cast<uint32_t>(std::min(dist_to_read_end, dist_to_contig_end));
        ...
        if (skip_dist > 1) {
          ...
          if (neighbor_dist < skip_dist) {
            bool found_match = check_direct_match<false>(
              skip_ctx, k, direction, neighbor_dist, cCurrPos, raw_hits);
            if (!found_match) {
          ...
          if constexpr (verbose) {
            std::cerr << "skip_dist: " << skip_dist
                      << ", actual_skip: " << actual_dist << "\n";
          }
          ...
          if (actual_dist == skip_dist) {
            bool found_match = check_direct_match<true>(
              skip_ctx, k, direction, skip_dist, cCurrPos, raw_hits);
            if (found_match) {
              continue;
            }
          }
          ...
          if constexpr (verbose) {
            std::cerr << "query at read_pos: " << skip_ctx.read_pos() << " "
                      << (alt_found ? "was" : "was not")
                      << " found in the index\n";
          }
          ...
          if (alt_found) {
            ...
            bool accept_hit =
              (check_phit.contig_id() == phit.contig_id()) and
              (check_phit.hit_fw_on_contig() == phit.hit_fw_on_contig()) and
              ((direction > 0) ? (check_phit.contig_pos() > phit.contig_pos())
            ...
            if (!skip_ctx.is_exhausted()) {
              ...
              if (mid_found) {
                ...
                if (mid_phit.contig_id() == phit.contig_id()) {
                  ...
                  mid_acceptable = true;
                } else if (alt_found and
                           mid_phit.contig_id() == alt_phit.contig_id()) {
                  // matched our second contig
                  // in this case, no point in adding the mid hit (it's
                  // redundant), just add the alt_phit.
                  alt_phit.resulted_from_open_search = true;
                  raw_hits.push_back({alt_kit->second, alt_phit});
                  mid_acceptable = true;
                } else {
...
bool hit_searcher::get_raw_hits_sketch_everykmer(std::string &read,
  ...
  //  2) The immedidate contig reference kmer does not match the read kmer
  //  following the previous hit 3) The distance to contig end is 0
  // At each kmer check we are resetting the phit variables, where the new_state
  // is also defined size_t size_hits = 0;
  while (kit != kit_end) {
