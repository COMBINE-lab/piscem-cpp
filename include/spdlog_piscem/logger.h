...
#ifndef SPDLOG_PISCEM_NO_EXCEPTIONS
#    define SPDLOG_PISCEM_LOGGER_CATCH(location)                                                                                                  \
        catch (const std::exception &ex)                                                                                                   \
        {                                                                                                                                  \
            if (location.filename)                                                                                                         \
            {                                                                                                                              \
                err_handler_(fmt_lib::format(SPDLOG_PISCEM_FMT_STRING("{} [{}({})]"), ex.what(), location.filename, location.line));              \
            }                                                                                                                              \
            else                                                                                                                           \
            {                                                                                                                              \
...
class SPDLOG_PISCEM_API logger
{
public:
    // Empty logger
    ...
    logger(logger &&other) SPDLOG_PISCEM_NOEXCEPT;
    logger &operator=(logger other) SPDLOG_PISCEM_NOEXCEPT;
    void swap(spdlog_piscem::logger &other) SPDLOG_PISCEM_NOEXCEPT;
...
protected:
    ...
    spdlog_piscem::level_t level_{level::info};
    spdlog_piscem::level_t flush_level_{level::off};
    ...
    void log_(source_loc loc, level::level_enum lvl, string_view_t fmt, Args &&... args)
    {
        ...
        SPDLOG_PISCEM_LOGGER_CATCH(loc)
    }
...
#    endif
    ...
    template<class T, typename std::enable_if<std::is_convertible<const T &, spdlog_piscem::wstring_view_t>::value, int>::type = 0>
