...
struct local_alloc_t
{
    ...
    SPDLOG_PISCEM_CONSTEXPR local_alloc_t() SPDLOG_PISCEM_NOEXCEPT : hlocal_(nullptr) {}

    local_alloc_t(local_alloc_t const &) = delete;
    local_alloc_t &operator=(local_alloc_t const &) = delete;

    ~local_alloc_t() SPDLOG_PISCEM_NOEXCEPT
    {
        if (hlocal_)
        {
            LocalFree(hlocal_);
...
public:
    ...
    static sid_t duplicate_sid(PSID psid)
    {
        ...
        {
            SPDLOG_PISCEM_THROW(win32_error("CopySid"));
        }
    ...
    static sid_t get_current_user_sid()
    {
        ...
        struct process_token_t
        {
            ...
            explicit process_token_t(HANDLE process)
            {
                ...
                {
                    SPDLOG_PISCEM_THROW(win32_error("OpenProcessToken"));
                }
            }
        ...
        {
            SPDLOG_PISCEM_THROW(win32_error("GetTokenInformation should fail"));
        }
        ...
        {
            SPDLOG_PISCEM_THROW(win32_error("GetTokenInformation"));
        }
...
private:
    ...
    HANDLE event_log_handle()
    {
        if (!hEventLog_)
        {
            ...
            {
                SPDLOG_PISCEM_THROW(internal::win32_error("RegisterEventSource"));
            }
        }
...
protected:
    void sink_it_(const details::log_msg &msg) override
    {
        ...
        if (!succeeded)
        {
            SPDLOG_PISCEM_THROW(win32_error("ReportEvent"));
        }
    }
