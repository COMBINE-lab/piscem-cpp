...
struct daily_filename_calculator
{
    ...
    static filename_t calc_filename(const filename_t &filename, const tm &now_tm)
    {
        ...
        return fmt_lib::format(SPDLOG_PISCEM_FMT_STRING(SPDLOG_PISCEM_FILENAME_T("{}_{:04d}-{:02d}-{:02d}{}")), basename, now_tm.tm_year + 1900,
 ...
 * Usages:
 *    auto sink =  std::make_shared<spdlog_piscem::sinks::daily_file_format_sink_mt>("myapp-%Y-%m-%d:%H:%M:%S.log", hour, minute);"
 *    auto logger = spdlog_piscem::daily_logger_format_mt("loggername, "myapp-%Y-%m-%d:%X.log", hour,  minute)"
 *
 */
struct daily_filename_format_calculator
{
    static filename_t calc_filename(const filename_t &filename, const tm &now_tm)
    {
        ...
        filename_t fmt_filename = fmt::format(SPDLOG_PISCEM_FMT_STRING(SPDLOG_PISCEM_FILENAME_T("{{:{}}}")), filename);

        // MSVC doesn't allow fmt::runtime(..) with wchar, with fmtlib versions < 9.1.x
#    if defined(_MSC_VER) && defined(SPDLOG_PISCEM_WCHAR_FILENAMES) && FMT_VERSION < 90101
        return fmt::format(fmt_filename, now_tm);
#    else
        return fmt::format(SPDLOG_PISCEM_FMT_RUNTIME(fmt_filename), now_tm);
#    endif
...
private:
    ...
    tm now_tm(log_clock::time_point tp)
    {
        ...
        return spdlog_piscem::details::os::localtime(tnow);
    }
...
// factory functions
//
template<typename Factory = spdlog_piscem::synchronous_factory>
