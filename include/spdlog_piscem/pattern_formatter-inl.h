...
#include <spdlog_piscem/details/fmt_helper.h>
...
SPDLOG_PISCEM_INLINE pattern_formatter::pattern_formatter(
...
SPDLOG_PISCEM_INLINE pattern_formatter::pattern_formatter(pattern_time_type time_type, std::string eol)
    : pattern_("%+")
    , eol_(std::move(eol))
    , pattern_time_type_(time_type)
...
SPDLOG_PISCEM_INLINE std::unique_ptr<formatter> pattern_formatter::clone() const
{
...
SPDLOG_PISCEM_INLINE void pattern_formatter::format(const details::log_msg &msg, memory_buf_t &dest)
{
    if (need_localtime_)
    {
...
SPDLOG_PISCEM_INLINE void pattern_formatter::set_pattern(std::string pattern)
{
    pattern_ = std::move(pattern);
    need_localtime_ = false;
    compile_pattern_(pattern_);
...
SPDLOG_PISCEM_INLINE void pattern_formatter::need_localtime(bool need)
{
    need_localtime_ = need;
}

SPDLOG_PISCEM_INLINE std::tm pattern_formatter::get_time_(const details::log_msg &msg)
{
...
SPDLOG_PISCEM_INLINE void pattern_formatter::handle_flag_(char flag, details::padding_info padding)
{
    ...
    default: // Unknown flag appears as is
        ...
        // spdlog_piscem::set_pattern("[%10!] %v") => "[      main] some message"
        // spdlog_piscem::set_pattern("[%3!!] %v") => "[mai] some message"
        else
        {
...
// Return padding.
SPDLOG_PISCEM_INLINE details::padding_info pattern_formatter::handle_padspec_(std::string::const_iterator &it, std::string::const_iterator end)
{
...
SPDLOG_PISCEM_INLINE void pattern_formatter::compile_pattern_(const std::string &pattern)
{
