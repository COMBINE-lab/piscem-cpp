...
private:
  ...
  static constexpr const uint16_t max_str_len{
...
public:
  ...
  rad_writer(std::vector<char> &&bin_data) : _bin_data(std::move(bin_data)){};
  ...
  rad_writer &operator<<(const bool &inval) {
    char *in_char_ptr =
      const_cast<char *>(reinterpret_cast<const char *>(&inval));
    std::copy(in_char_ptr, in_char_ptr + sizeof(inval),
  ...
  rad_writer &operator<<(const std::string &inval) {
    if (inval.size() >= max_str_len) {
      std::cerr << "ERROR!! DOESN'T SUPPORT STRING LENGTH LONGER THAN "
                << max_str_len << ". String length: " << inval.size() << "\n";
      std::exit(1);
    }
...
template bool rad_writer::write_integer_at_offset<uint16_t>(size_t offset,
                                                            const uint16_t &v);

template bool rad_writer::write_integer_at_offset<uint32_t>(size_t offset,
                                                            const uint32_t &v);

template bool rad_writer::write_integer_at_offset<uint64_t>(size_t offset,
                                                            const uint64_t &v);
